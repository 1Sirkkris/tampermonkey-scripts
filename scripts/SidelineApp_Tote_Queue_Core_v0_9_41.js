(() => {
  ...
  const PANEL_ID = 'tote-queue-panel-v0910';
  const STORE_KEY = 'sideline_v0910_tote_queue_text';
  ...
  function sourceScreenReady() {
    return !!scanInput() && /scan\s+source\s+container/i.test(document.body?.innerText || '');
  }
  ...
  function mount() {
    ...
    title.textContent = `Tote Queue v${VERSION}`;
    ...
    root.append(title, textarea, row, status, errors);
