// ==UserScript==
// @name         v1.1.0 SidelineApp Helper CLEAN TEST
// @namespace    https://github.com/1Sirkkris
// @version      1.1.0
// @description  Clean Sideline helper with feature parity: Tote Queue, Tote Scrubber warning, Qty, Lazy progress, expiry picker.
...
(() => {
  'use strict';
  if (window.__sidelineCleanTest_v110) return;
  window.__sidelineCleanTest_v110 = true;
  ...
  async function fillAndConfirm(value, expectedScreen = '') {
    ...
    const btn = confirmButton(); if (enabled(btn)) click(btn); else enter(input);
    return true;
  }
...
`;
  ...
  function applyPanels() {
    ...
    let bottom = 58;
    for (const key of ['lazy','scrub','queue']) { const panel = panels[key]; if (!panel || !feature[key]) continue; panel.style.bottom = `${bottom}px`; bottom += Math.max(80, panel.offsetHeight) + 10; }
    renderScrub();
  }
  ...
  function renderScrub() {
    ...
    let warning = $('#sh-scrub-warning');
    if (feature.scrub && !warning) { warning = document.createElement('div'); warning.id = 'sh-scrub-warning'; warning.textContent = '⚠ TOTE SCRUBBER ACTIVE — OPENED CONTAINERS WILL BE EMPTIED ⚠'; document.body.appendChild(warning); }
    if (!feature.scrub && warning) warning.remove();
  }
  async function scrubTick() {
    if (!feature.scrub || state.scrubBusy || state.owner === 'queue' || state.owner === 'lazy') return;
    const change = changeButton(); if (!change) return;
  ...
  async function queuePump() {
    ...
    try { if (screen() !== 'SOURCE' && changeButton()) { const r=await clearOpenContainer(); if(r!=='cleared') throw new Error(r); } renderQueue('scanning source'); if (!await fillAndConfirm(code,'SOURCE')) throw new Error('scan timeout'); const result=await clearOpenContainer(); if(result!=='cleared') throw new Error(result); q.index++; renderQueue('cleared'); }
  ...
  function renderLazy(note=''){
    const cur=lazy.items[lazy.index]; const total=lazy.items.reduce((s,x)=>s+x.qty,0); const unique=lazy.items.length; const remaining=Math.max(0,unique-lazy.index);
    mTotal.textContent=total; mUnique.textContent=unique; mRemaining.textContent=remaining;
    lStatus.textContent=`${lazy.running?(lazy.paused?'PAUSED':'RUNNING'):'IDLE'} | ${lazy.stage}${cur?` | ${cur.code} x${cur.qty}`:''}${note?' | '+note:''}`; lError.textContent=lazy.error;
  ...
  function installContainerAdvance(input,next){
    input.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!=='Tab')return;e.preventDefault();const v=input.value.trim();if(!validContainer(v)){lazy.error='Container must start with csX or tsX.';input.value='';renderLazy();return;}lazy.error='';setTimeout(()=>next.focus(),0);renderLazy();});
    input.addEventListener('paste',()=>setTimeout(()=>{if(validContainer(input.value.trim())){lazy.error='';next.focus();}else{lazy.error='Container must start with csX or tsX.';input.value='';}renderLazy();},0));
  }
  installContainerAdvance(lSrc,lDest); installContainerAdvance(lDest,lItems);
  lItems.addEventListener('keydown',e=>{if(e.key==='Enter'){setTimeout(()=>{if(!lItems.value.endsWith('\n'))lItems.value+='\n';refreshItems();},0);}});
  ...
  function expiryInputs(){
    const inputs=appElements('input,textarea').filter(visible);
    const hint=re=>inputs.find(el=>re.test(norm(`${el.name||''} ${el.id||''} ${el.placeholder||''} ${el.getAttribute('aria-label')||''}`)));
    const month=hint(/\b(mm|month)\b/),day=hint(/\b(dd|day)\b/),year=hint(/\b(yyyy|year)\b/);
    if(month&&day&&year)return{month,day,year};
    const text=inputs.filter(el=>['','text','tel','number','search'].includes(norm(el.type||'')));
    return text.length>=3?{month:text[0],day:text[1],year:text[2]}:null;
  }
  ...
  function renderExpiry(){
    const show=screen()==='EXPIRY'; expiryPanel.style.display=show?'block':'none'; if(!show)return;
    const now=new Date();const years=Array.from({length:16},(_,i)=>now.getFullYear()+i);
    expiryPanel.innerHTML=`<div class="sh-title">Expiry helper v${VERSION}</div><div class="sh-expiry-section"><div class="sh-expiry-label">Month</div><div class="sh-expiry-grid">${makeButtons(Array.from({length:12},(_,i)=>String(i+1).padStart(2,'0')),'m')}</div></div><div class="sh-expiry-section"><div class="sh-expiry-label">Day</div><div class="sh-expiry-grid">${makeButtons(Array.from({length:31},(_,i)=>String(i+1).padStart(2,'0')),'d')}</div></div><div class="sh-expiry-section"><div class="sh-expiry-label">Year</div><div class="sh-expiry-grid">${makeButtons(years,'y')}</div></div><button class="sh-btn sh-pao" data-exp="pao">PAO +900 DAYS</button>`;
    const rightBottom=feature.lazy?58+Math.max(80,lazyPanel.offsetHeight)+10:58;expiryPanel.style.bottom=`${rightBottom}px`;
  }
  expiryPanel.addEventListener('click',async e=>{const b=e.target.closest('[data-exp]');if(!b||state.expiryBusy)return;const inputs=expiryInputs();if(!inputs)return;state.expiryBusy=true;try{const a=b.dataset.exp,v=b.dataset.v;if(a==='m')fillExpiryPart(inputs.month,v);if(a==='d')fillExpiryPart(inputs.day,v);if(a==='y'){fillExpiryPart(inputs.year,v);await sleep(40);const c=confirmButton();if(c)click(c);else enter(inputs.year);}if(a==='pao'){const d=new Date();d.setDate(d.getDate()+900);fillExpiryPart(inputs.month,String(d.getMonth()+1).padStart(2,'0'));fillExpiryPart(inputs.day,String(d.getDate()).padStart(2,'0'));fillExpiryPart(inputs.year,String(d.getFullYear()));await sleep(40);const c=confirmButton();if(c)click(c);else enter(inputs.year);}}finally{setTimeout(()=>{state.expiryBusy=false;},150);}});
