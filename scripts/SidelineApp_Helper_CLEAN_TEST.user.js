// ==UserScript==
// @name         v1.0.1 SidelineApp Helper CLEAN TEST
...
(() => {
  'use strict';
  if (window.__sidelineCleanTest_v101) return;
  window.__sidelineCleanTest_v101 = true;
  ...
  const helperSelector = '#sh-dock,#sh-queue,#sh-scrub,#sh-qty,#sh-lazy,#sh-scrub-warning';
  ...
  function appElements(sel) { return $$(sel).filter(el => !el.closest(helperSelector)); }
  ...
  function confirmButton() {
    ...
    return enabled(byId) && !byId.closest(helperSelector) ? byId : buttonByText(/^(confirm|item match|continue|submit|enter)\b/);
  }
  function changeButton() {
    const byId = $('#change-container-button');
    return enabled(byId) && !byId.closest(helperSelector) ? byId : buttonByText(/change container/);
  }
  function modalButton(choice) {
    const wanted = norm(choice);
    const roots = [$('#modal-root'), ...$$('[role="dialog"],dialog,.modal,.Dialog,.dialog,.ReactModal__Content')].filter(Boolean);
    ...
    for (const root of roots) { const hit = find(root); if (hit) return hit; }
  ...
  async function waitFor(test, timeout = 10000, gap = 50) {
    ...
    while (Date.now() < end) { const v = test(); if (v) return v; await sleep(gap); }
    return null;
  }
...
@keyframes shDangerFlash{0%,100%{background:#b91c1c;color:#fff}50%{background:#fff200;color:#111}}
#sh-scrub-warning{position:fixed;left:0;right:0;bottom:0;z-index:2147483645;padding:12px 16px;text-align:center;font:900 16px Arial,sans-serif;letter-spacing:.5px;border-top:3px solid #fff;box-shadow:0 -3px 12px #0005;pointer-events:none;animation:shDangerFlash .75s infinite}
`;
  ...
  function showScrubWarning() {
    ...
    if (feature.scrub) {
      if (!warning) { warning = document.createElement('div'); warning.id = 'sh-scrub-warning'; warning.textContent = '⚠ TOTE SCRUBBER ACTIVE — OPENED CONTAINERS WILL BE EMPTIED ⚠'; document.body.appendChild(warning); }
    } else warning?.remove();
  }
  ...
  async function queuePump(){if(!q.running||q.paused||state.queueBusy)return;if(q.index>=q.list.length){q.running=false;state.owner='';renderQueue('done');return;}state.queueBusy=true;state.owner='queue';const code=q.list[q.index];try{if(screen()!=='SOURCE'&&changeButton()){const r=await clearOpenContainer();if(r!=='cleared')throw new Error(r);}renderQueue('scanning source');if(!await fillAndConfirm(code,'SOURCE'))throw new Error('scan timeout');const r=await clearOpenContainer();if(r!=='cleared')throw new Error(r);q.index++;renderQueue('cleared');}catch(e){q.failed.push(`${code} (${e.message})`);q.index++;renderQueue(e.message);}finally{state.queueBusy=false;state.owner='';if(q.running&&!q.paused)setTimeout(queuePump,40);}}
  ...
  lazyPanel.insertAdjacentHTML('beforeend',`<div class="sh-row"><input class="sh-input" data-f="src" placeholder="Source container (csX / tsX)"><input class="sh-input" data-f="dest" placeholder="Destination container (csX / tsX)"></div><textarea class="sh-input sh-area" data-f="items" placeholder="Scan barcodes — one per line; duplicates become quantity"></textarea><label style="display:block;margin:7px 0"><input type="checkbox" data-f="clear"> Clear source when done</label><div class="sh-grid4"><button class="sh-btn sh-on" data-a="start">Start</button><button class="sh-btn" data-a="pause">Pause</button><button class="sh-btn sh-stop" data-a="stop">Stop</button><button class="sh-btn" data-a="reset">Reset</button></div><div class="sh-status"></div><div class="sh-error"></div>`);
  ...
  function rejectContainer(input,label){input.value='';input.style.borderColor='#dc2626';lazy.error=`${label} must start with csX or tsX.`;renderLazy();input.focus();}
  function acceptContainer(input,label,next){const v=input.value.trim();if(!validContainer(v)){rejectContainer(input,label);return false;}input.style.borderColor='#16a34a';lazy.error='';renderLazy();setTimeout(()=>next.focus(),0);return true;}
  for(const [input,label,next] of [[lSrc,'SRC',lDest],[lDest,'DEST',lItems]]){
    input.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!=='Tab')return;e.preventDefault();e.stopPropagation();acceptContainer(input,label,next);},true);
    input.addEventListener('change',()=>{if(input.value.trim())acceptContainer(input,label,next);});
    input.addEventListener('paste',()=>setTimeout(()=>{if(input.value.trim())acceptContainer(input,label,next);},0));
  }
  lItems.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!=='Tab')return;e.preventDefault();e.stopPropagation();const value=lItems.value.trim();if(value&&!lItems.value.endsWith('\n'))lItems.value+='\n';lItems.dispatchEvent(new Event('input',{bubbles:true}));},true);
  lItems.addEventListener('input',()=>{const lines=lItems.value.split(/\r?\n/).map(v=>v.trim()).filter(Boolean);lItems.value=lines.join('\n')+(lines.length?'\n':'');if(!lazy.running){lazy.items=parseItems(lItems.value);renderLazy();}});
