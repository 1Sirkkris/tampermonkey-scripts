// ==UserScript==
// @name         v1.3.0 SidelineApp Helper CLEAN TEST
// @namespace    https://github.com/1Sirkkris
// @version      1.3.0
// @description  Clean Sideline helper with faster screen detection, Tote Queue, Scrubber warning, Qty, Lazy progress, full Predicant recovery and expiry picker.
...
(() => {
  'use strict';
  if (window.__sidelineCleanTest_v130) return;
  window.__sidelineCleanTest_v130 = true;
  ...
  async function clearOpenContainer() { return closeOpenContainer('yes'); }
...
.sh-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:8px 0}.sh-metric{border:1px solid #c7d0dd;background:#f8fafc;padding:8px;text-align:center}.sh-metric b{display:block;font-size:26px;line-height:1}.sh-progress{max-height:180px;overflow:auto;border-top:1px solid #d5dbe3;margin-top:8px;padding-top:6px;font-family:Consolas,monospace}.sh-item{padding:4px 6px;border-radius:3px}.sh-item.current{background:#fff3bf;border:1px solid #f59e0b;font-weight:900}.sh-expiry-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin-top:5px}.sh-expiry-grid button{padding:6px 4px}.sh-expiry-section{margin:8px 0}.sh-expiry-label{font-weight:800;margin-bottom:4px}.sh-pao{width:100%;margin-top:8px;background:#7c3aed!important;color:#fff!important;border-color:#6d28d9!important}`;
  ...
  function panel(id,title,key) {
    ...
    root.id=id; root.className='sh-panel'; root.innerHTML=`<div class="sh-title">${title}</div>`; root.style.display='none';
    document.body.appendChild(root); panels[key]=root; return root;
  }
  function mountDock() {
    if ($('#sh-dock')) return;
  ...
  function renderScrub() {
    scrubStatus.textContent=feature.scrub?'ACTIVE — Change container → Yes':'OFF';
    let w=$('#sh-scrub-warning');
    if(feature.scrub&&!w){w=document.createElement('div');w.id='sh-scrub-warning';w.textContent='⚠ TOTE SCRUBBER ACTIVE — OPENED CONTAINERS WILL BE EMPTIED ⚠';document.body.appendChild(w);}
  ...
  async function scrubTick() {
    ...
    if(!change)return;
    state.scrubBusy=true;state.owner='scrub';
  ...
  async function queuePump() {
    ...
    state.queueBusy=true;state.owner='queue';
    const code=q.list[q.index];
    try{
      if(screen()!=='SOURCE'&&changeButton()){const r=await clearOpenContainer();if(r!=='closed')throw new Error(r);}
  ...
  queuePanel.onclick=e=>{
    ...
    if(!a)return;
    if(a==='start'){q.list=parseContainers(qText.value);q.index=0;q.failed=[];q.running=!!q.list.length;q.paused=false;renderQueue(q.running?'starting':'no containers');queuePump();}
    if(a==='pause'){q.paused=!q.paused;e.target.textContent=q.paused?'Resume':'Pause';renderQueue();if(!q.paused)queuePump();}
    if(a==='stop'){q.running=false;q.paused=false;state.owner='';renderQueue('stopped');}
  };
  renderQueue();
  ...
  function renderLazy(note=''){
    const cur=lazy.items[lazy.index],total=lazy.items.reduce((s,x)=>s+x.qty,0),unique=lazy.items.length,remaining=Math.max(0,unique-lazy.index);
    mTotal.textContent=total;mUnique.textContent=unique;mRemaining.textContent=remaining;
    const mode=lazy.recovering?'RECOVERING':lazy.predicant?'PREDICANT':lazy.running?(lazy.paused?'PAUSED':'RUNNING'):'IDLE';
    lStatus.textContent=`${mode} | ${lazy.stage}${cur?` | ${cur.code} x${cur.qty}`:''}${note?' | '+note:''}`;
    lError.textContent=lazy.error;
    lProgress.innerHTML=lazy.items.map((it,i)=>`<div class="sh-item ${i===lazy.index&&lazy.running?'current':''}">${i<lazy.index?'✓ ':i===lazy.index&&lazy.running?'▶ ':'• '}${it.code} ×${it.qty}</div>`).join('');
  }
  ...
  async function recoverPredicant() {
    ...
    lazy.recovering=true;lazy.error='Recovery: returning to original source…';renderLazy();state.owner='lazy';
    try{
      const back=await waitFor(()=>buttonByText(/back to source/),6000,40);
      if(back)click(back);
      const sourceChange=await waitFor(changeButton,12000,40);
      if(!sourceChange)throw new Error('original source not open');
      lazy.error='Recovery: closing original source without emptying…';renderLazy();
      const sourceResult=await closeOpenContainer('no');
      if(sourceResult!=='closed')throw new Error(sourceResult);

      lazy.error=`Recovery: opening destination ${lazy.dest} as source…`;renderLazy();
      if(!await fillAndConfirm(lazy.dest,'SOURCE'))throw new Error('destination source scan timeout');
      lazy.error='Recovery: emptying destination container…';renderLazy();
      const destResult=await clearOpenContainer();
      if(destResult!=='closed')throw new Error(destResult);

      lazy.predicant=false;lazy.recovering=false;lazy.paused=false;lazy.stage='SOURCE';lazy.error='';
      renderLazy('destination emptied — retrying current item');
      state.owner='lazy';
    }catch(e){
      lazy.recovering=false;lazy.paused=true;lazy.error=`Predicant recovery failed: ${e.message}. Rescan DEST to retry.`;
      renderLazy();state.owner='';
    }
  }
  ...
  async function lazyTick() {
    ...
    const s=screen();
    if(s==='PREDICANT'){
      lazy.predicant=true;lazy.paused=true;
      lazy.error=`Predicant detected — rescan destination ${lazy.dest} to recover.`;
      renderLazy();state.owner='';return;
    }
    ...
    if(!item){
      ...
      try{
        if(lClear.checked&&changeButton()){const result=await clearOpenContainer();if(result!=='closed')throw new Error(result);}
        resetLazyReady('complete — ready for next round');
    ...
    try{
      ...
      if(lazy.stage==='SOURCE'){
        if(s==='SOURCE')await fillAndConfirm(lazy.src,'SOURCE');
        else if(['ITEM','VERIFY','QTY','DEST'].includes(s))lazy.stage='ITEM';
      }else if(lazy.stage==='ITEM'){
        if(s==='ITEM'){await fillAndConfirm(item.code,'ITEM');lazy.stage='VERIFY';}
        else if(s==='VERIFY')lazy.stage='VERIFY';
        else if(s==='SOURCE')lazy.stage='SOURCE';
      }else if(lazy.stage==='VERIFY'){
        if(s==='VERIFY'){const b=confirmButton();if(b)click(b);}
        else if(s==='QTY')lazy.stage='QTY';
        else if(s==='DEST')lazy.stage='DEST';
      }else if(lazy.stage==='QTY'){
        if(s==='QTY'){
          const input=scanInput();
          if(input){input.focus();input.select?.();setValue(input,'');await sleep(10);setValue(input,item.qty);await sleep(20);const b=confirmButton();enabled(b)?click(b):enter(input);}
        }else if(s==='DEST')lazy.stage='DEST';
      }else if(lazy.stage==='DEST'){
        if(s==='DEST'){await fillAndConfirm(lazy.dest,'DEST');lazy.stage='ADVANCE';}
      }else if(lazy.stage==='ADVANCE'&&['ITEM','SUCCESS','SOURCE'].includes(s)){
        lazy.index++;lazy.stage=s==='SOURCE'?'SOURCE':'ITEM';lazy.error='';
      }
      renderLazy();
  ...
  lazyPanel.onclick=e=>{
    ...
    if(!a)return;
    if(a==='start'){
      lazy.src=lSrc.value.trim();lazy.dest=lDest.value.trim();lazy.items=parseItems(lItems.value);lazy.index=0;lazy.error='';lazy.predicant=false;lazy.recovering=false;
      if(!validContainer(lazy.src)||!validContainer(lazy.dest)){lazy.error='SRC and DEST must start with csX or tsX.';renderLazy();return;}
      if(!lazy.items.length){lazy.error='No item barcodes.';renderLazy();return;}
      lazy.running=true;lazy.paused=false;lazy.stage='SOURCE';renderLazy('started');
    }
    if(a==='pause'){if(lazy.predicant||lazy.recovering)return;lazy.paused=!lazy.paused;e.target.textContent=lazy.paused?'Resume':'Pause';if(!lazy.paused){lazy.error='';state.owner='lazy';}else state.owner='';renderLazy();}
    if(a==='stop'){lazy.running=false;lazy.paused=false;lazy.predicant=false;lazy.recovering=false;lazy.stage='IDLE';state.owner='';renderLazy('stopped');}
    if(a==='reset')resetLazyReady('reset');
  };
  renderLazy();
  ...
  function boot(){
    mountDock();applyPanels();renderScrub();
    const observer=new MutationObserver(()=>{screenDirty=true;});
    ...
    setInterval(()=>{screenDirty=true;},1000);
    setInterval(scrubTick,120);
    setInterval(lazyTick,140);
    setInterval(renderExpiry,300);
  }
